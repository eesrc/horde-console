...
export class Pagination {
  ...
  filter() {
    ...
    Log.debug("filtering");
    this.filteredCollection = collection.splice(
      parseInt(this.rowsPrPage, 10) * this.offset,
      parseInt(this.rowsPrPage, 10),
    );
    this.filterCallback({
  ...
  bind() {
    this.filter();
    this.subscription = this.bindingEngine.collectionObserver(this.collection).subscribe(() => {
